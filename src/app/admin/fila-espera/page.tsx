      ...
      <div className="max-w-4xl mx-auto px-0 md:px-6 py-5 space-y-5">
        ...
        <div className="card overflow-hidden p-0">
          ...
          {loadingFila ? (
            ...
            <div>
              <div className="hidden md:grid grid-cols-12 gap-3 px-5 py-2.5 bg-gray-50 border-b border-gray-100 text-xs font-semibold text-gray-500 uppercase tracking-wide">
              ...
              {fila.map((r) => (
                <Fragment key={r.id}>
                  {/* Mobile: linha vira card */}
                  <div className="md:hidden px-5 py-3 border-b border-gray-50">
                    <div className="flex items-center justify-between gap-3">
                      <div className="min-w-0 text-sm font-medium text-gray-900 truncate">{r.cliente}</div>
                      <div className="flex-shrink-0 text-xs text-gray-400">{tempoRelativo(r.confirmado_em)}</div>
                    </div>
                    <div className="text-sm text-gray-700 mt-0.5">
                      <span className="font-medium">{r.modalidade}</span>
                      <span className="text-gray-400"> · {formatarData(r.data)} {r.horario}</span>
                    </div>
                    <div className="text-xs text-gray-600 mt-1 truncate">{r.unidade} · {r.plano}</div>
                  </div>
                  <div className="hidden md:grid grid-cols-12 gap-3 px-5 py-3 border-b border-gray-50 hover:bg-gray-50 transition-colors items-center">
              ...
              {fila.map((r) => (
                <Fragment key={r.id}>
                  ...
                  <div className="md:hidden px-5 py-3 border-b border-gray-50">
                    <div className="flex items-center justify-between gap-3">
                      <div className="min-w-0 text-sm font-medium text-gray-900 truncate">{r.cliente}</div>
                      <span className="flex-shrink-0 inline-flex items-center justify-center min-w-[1.75rem] px-2 py-0.5 rounded-full bg-primary-50 text-primary-700 text-xs font-bold">
                        {r.posicao}º
                      </span>
                    </div>
                    <div className="text-sm text-gray-700 mt-0.5">
                    ...
                    <div className="flex items-center justify-between gap-3 mt-1 text-xs">
                      <div className="min-w-0 text-gray-600 truncate">{r.unidade} · {r.plano}</div>
                      <div className="flex-shrink-0 text-gray-400">{tempoRelativo(r.criado_em)}</div>
                    </div>
                  </div>
                  <div className="hidden md:grid grid-cols-12 gap-3 px-5 py-3 border-b border-gray-50 hover:bg-gray-50 transition-colors items-center">
                    <div className="col-span-3 text-sm font-medium text-gray-900 truncate">{r.cliente}</div>
