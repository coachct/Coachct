...
// ─────────────────────────────────────────────────────────────────────────────
// Relatório — Reservas feitas pelo app da TotalPass (Club).
// Lista as club_reservas com totalpass_slot_id preenchido (= vieram do app deles),
// no período escolhido (pela data da aula). Filtro por status + export CSV.
// Só-leitura; não toca em nada do fluxo.
// ─────────────────────────────────────────────────────────────────────────────
...
export default function ReservasTotalpassPage() {
  ...
  async function carregar() {
    ...
    // 2) Reservas TotalPass (totalpass_slot_id != null) dessas ocorrências.
    const reservas: any[] = []
    ...
    const arr: Linha[] = reservas.map((r: any) => {
      ...
      return {
        ...
        unidade: oc.unidade,
        cliente: r.clientes?.nome || 'Cliente TotalPass',
  ...
  function baixarCsv() {
    ...
    a.href = url
    a.download = `reservas-totalpass-${de}_a_${ate}.csv`
    a.click()
    ...
    <div>
      ...
      <div className="grid grid-cols-2 items-end gap-3 mb-5 md:flex md:flex-wrap">
        ...
        <label className="text-xs text-gray-500 min-w-0 md:min-w-[auto]">
          ...
          <select value={unidadeFiltro} onChange={e => setUnidadeFiltro(e.target.value)} className="input text-base md:text-sm">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <KpiCard label="Reservas TotalPass" value={String(kpis.total)} sub="no período/filtro" subColor="text-primary-600" />
      ...
      {carregando ? (
        ...
        <div className="text-center py-10 text-sm text-gray-400">Nenhuma reserva TotalPass neste período/filtro.</div>
      ) : (
        <div className="card">
