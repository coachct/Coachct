...
export async function POST(req: NextRequest) {
  try {
    ...
    const dataAula    = reserva.club_ocorrencias?.data || ''
    const horarioAula = (reserva.club_ocorrencias?.club_aulas?.horario || '').slice(0, 5)
    const unidadeId   = reserva.club_ocorrencias?.club_aulas?.unidade_id || null
    const unidadeNome = reserva.club_ocorrencias?.club_aulas?.unidades?.nome || 'JustClub'
