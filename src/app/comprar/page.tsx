...
export default function ComprarPage() {
  ...
  async function carregarProdutos() {
    setLoading(true)
    const { data } = await supabase.from('produtos').select('*').eq('ativo', true).or(`unidade_id.eq.${JUST_CT_ID},unidade_id.is.null`).order('valor', { ascending: false })
    setProdutos(data || [])
    setLoading(false)
  }
  ...
  async function verificarCoachCtProAtivo() {
    ...
    if (!cliente) return
    const hoje = new Date()
    const { data: saldoData } = await supabase.rpc('saldo_creditos_cliente', { p_cliente_id: cliente.id, p_mes: hoje.getMonth() + 1, p_ano: hoje.getFullYear(), p_unidade_id: null })
  ...
  function descricaoProduto(p: any): string {
    if (p.subtipo === 'acesso') return `Acesso ilimitado ao espaço de musculação Just CT por ${p.dias_validade} dias. Válido somente para o titular.`
  ...
  // Calcula valor mensal e total para produtos de acesso
  function valorMensalAcesso(p: any): { mensal: number; total: number; meses: number } {
  ...
  return (
    <div style={{ background: '#080808', minHeight: '100vh', color: '#f0f0f0', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ paddingTop: 120, paddingBottom: '3rem', padding: '120px 2.5rem 3rem', maxWidth: 1100, margin: '0 auto', textAlign: 'center' as const }}>
        <div style={{ fontSize: 11, textTransform: 'uppercase' as const, letterSpacing: 3, color: ACCENT, fontFamily: "'DM Mono', monospace", marginBottom: '1rem' }}>// comprar online</div>
        <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 'clamp(48px, 6vw, 72px)', color: '#fff', lineHeight: 1.05, marginBottom: '1rem', letterSpacing: 2 }}>PLANOS</div>
      ...
      <div style={{ padding: '0 2.5rem 6rem', maxWidth: 1100, margin: '0 auto' }}>
        {loading ? (
          ...
          <>
                  ...
                  <div style={{ background: `${ACCENT}10`, border: `1.5px solid ${ACCENT}66`, borderRadius: 12, padding: '1.25rem 1.5rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 13, color: '#aaa', lineHeight: 1.5 }}>
                        Plano válido até <strong style={{ color: '#fff' }}>{formatarData(coachCtProAtivo.fim)}</strong> · {coachCtProAtivo.disponivel} de {coachCtProAtivo.total} créditos disponíveis.<br />
                        Para adquirir um novo, aguarde o término dos créditos ou do plano.
                      </div>
                    </div>
                  </div>
                )}
                  ...
                  {produtosCoachPro.map(p => {
                    ...
                    return (
                      <div key={p.id} className="coach-pro-card" style={{ position: 'relative' as const, background: promo ? `linear-gradient(135deg, #111 0%, #1a0a14 100%)` : '#111', border: `1.5px solid ${promo ? ACCENT : '#2a2a2a'}`, borderRadius: 16, padding: '2rem', display: 'flex', flexDirection: 'column' as const, overflow: 'hidden' as const, opacity: bloqueado ? 0.55 : 1 }}>
                        {promo && (
                          <div style={{ position: 'absolute' as const, top: 16, right: -38, background: ACCENT, color: '#fff', fontSize: 10, fontWeight: 700, padding: '0.3rem 3rem', transform: 'rotate(38deg)', letterSpacing: 1.5, fontFamily: "'DM Mono', monospace" }}>OFERTA LANÇAMENTO</div>
                        )}
                        <div style={{ marginBottom: '1.5rem' }}>
            ...
            {/* ACESSO — valor mensal em destaque igual à home */}
                  ...
                  {produtosAcesso.map(p => {
                    ...
                    return (
                      <div key={p.id} className="produto-card-h" style={{ background: '#111', border: `1px solid ${badge?.isMaisPopular ? ACCENT : '#222'}`, borderRadius: 16, padding: '2rem', position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                        {badge && (
                          <div style={{ position: 'absolute', top: 12, right: -16, background: ACCENT, color: '#fff', fontSize: 10, fontWeight: 700, padding: '0.25rem 2.5rem', transform: 'rotate(15deg)', letterSpacing: 1 }}>{badge.label}</div>
                        )}
                        ...
                        <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem', marginBottom: 4 }}>
                          <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 48, color: '#fff', lineHeight: 1 }}>
                            {valorMensalFmt.reais}<span style={{ fontSize: 24 }}>{valorMensalFmt.cents}</span>
                          </div>
                          <div style={{ fontSize: 14, color: '#999', fontWeight: 500 }}>/mês</div>
                        </div>
                        {/* Total e período abaixo */}
                        <div style={{ fontSize: 12, color: '#555', marginBottom: '1rem', fontFamily: "'DM Mono', monospace" }}>
                          {totalFmt.reais}{totalFmt.cents} total · {meses}x
                        </div>
