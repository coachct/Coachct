...
export default function RecepcaoPosicoesPage() {
  ...
  async function carregarUnidades() {
    // Recepção: só unidades vinculadas via perfil_unidades; admin/coord: todas
    if (perfil?.role === 'recepcao') {
      const { data: pu } = await supabase
        .from('perfil_unidades')
        .select('unidade_id, unidades(id, nome, tipo, ativo)')
        .eq('perfil_id', perfil.id)
      const clubUnidades = (pu || [])
        .map((p: any) => p.unidades)
        .filter((u: any) => u?.tipo === 'club' && u?.ativo)
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-4">
        ...
        {unidades.length === 0 && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-6 text-center text-sm text-amber-700">
            Sua conta não está vinculada a nenhuma unidade JustClub.
          </div>
        )}
