...
// TEXTO PROVISÓRIO/aprovado: os textos longos são os do canvas citados no brief.
const TEXTO_LONGO: Record<number, { chapeu: string; texto: string }> = {
  15: {
    ...
    texto:
      ...
      'pra usar quando quiser, em qualquer unidade. Ainda não treina? É a porta de entrada pro ' +
      'projeto. Quinze treinos, no ritmo que couber na sua semana.',
  },
  30: {
  ...
  return (
    <>
      <style>{`
        ...
        .sm-selo-desk { display: block; }
        .sm-selo-mob  { display: none; }
        @media (max-width: 900px) {
          .sm-grid { grid-template-columns: 1fr !important; }
          .sm-destaque { order: -1; }
          .sm-selo-desk { display: none; }
          .sm-selo-mob  { display: inline-block; }
        }
      `}</style>
        ...
        {lista.map(p => {
          ...
          return (
            ...
            >
              {destaque && (
                <>
                  <div className="sm-selo-desk" style={{
                    position: 'absolute', top: 16, right: -38, background: ACCENT, color: '#fff',
                    fontSize: 10, fontWeight: 700, padding: '0.3rem 3rem', transform: 'rotate(38deg)',
                    letterSpacing: 1.5, fontFamily: "'DM Mono', monospace",
                  }}>PROJETO INTEIRO</div>
                  <div className="sm-selo-mob" style={{
                    position: 'absolute', top: 14, right: 14, background: ACCENT, color: '#fff',
                    fontSize: 9, fontWeight: 700, padding: '0.25rem 0.7rem', borderRadius: 20,
                    letterSpacing: 1.2, fontFamily: "'DM Mono', monospace",
                  }}>PROJETO INTEIRO</div>
                </>
              )}

              <div style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: 2, color: ACCENT, marginBottom: '0.4rem', fontFamily: "'DM Mono', monospace" }}>
                summer mode
              </div>
              ...
              {variante === 'longo' && longo && (
                <div style={{ fontSize: 13, color: ACCENT, fontStyle: 'italic', marginTop: '0.5rem' }}>
                  {longo.chapeu}
                </div>
              )}

              <div style={{ fontSize: 14, color: '#999', lineHeight: 1.7, margin: '0.85rem 0 1.5rem' }}>
              ...
              <div style={{ fontSize: 12, color: '#777', marginBottom: '1.5rem', fontFamily: "'DM Mono', monospace" }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem', marginBottom: '2rem', flex: 1 }}>
                {[
                  'Vale nos JustClub e na musculação livre do Just CT',
                  `Válido até ${dataBR(p.validade_fixa)}`,
                  bonus > 0 ? `Fecha os ${creditos} até 31/12? Ganha +${bonus}.` : null,
                ].filter(Boolean).map((b, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.6rem' }}>
                    <div style={{ width: 17, height: 17, borderRadius: '50%', background: `${ACCENT}25`, color: ACCENT, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, flexShrink: 0, marginTop: 2 }}>✓</div>
                    <span style={{ fontSize: 13, color: '#ddd', lineHeight: 1.5 }}>{b}</span>
                  </div>
                ))}
              </div>
